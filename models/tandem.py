...
class Model(GenericModel):
    def __init__(self, state_dim: int, action_dim: int):
        ...
        self.CR = 1.0
        self.discount = 0.9
        print("Discount is set to {}. Change it ?".format(self.discount))
        self.lambda_tilde = (
        ...
        self.action_dim = int(self.actions_per_server**2)
    ...
    def build_p(self):
        for a in range(self.action_dim):
            for s in range(self.state_dim):
                state, action = self.state_list[s], self.action_decoding[a]
                m1, m2, k1, k2 = state
                a1, a2 = action
