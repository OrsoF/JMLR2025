...
class Solver:
    def __init__(
        ...
        self.epsilon_pbr = self.epsilon / 2  # Bound on the projected bellman residual
        self.epsilon_span = self.epsilon / 2  # Bound on the span of bellman(V)

    def run(
        self,
    ):
        start_time = time()
        contracted_value = np.zeros((len(self.partition.states_in_region)))

        while True:
            if self.partition._number_of_regions() > 0.9 * self.model.state_dim:
                value = self.partition._partial_phi().dot(contracted_value)
                value = apply_obo_until_var_small(
            ...
            bellman_of_value = self._exact_bellman_step(contracted_value)
            contracted_value = self.partition.weights.dot(bellman_of_value)
            maximum_span = max(
            ...
            if maximum_span > self.epsilon_span:
                ...
                contracted_value = self.partition.divide_all_regions_along_value_update_contracted_value(
                    bellman_of_value,
                    self.epsilon_span,
                    contracted_value,
                )
            ...
            if maximum_span < self.epsilon_span and pbr_value < self.epsilon_pbr:
                self.runtime = time() - start_time
                self.value = self.partition._partial_phi().dot(contracted_value)
                break
